...
from donut import DonutMultiTaskDataset
...
class BinModelCheckpoint(Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        print('save binmodel!!!!')
        save_path = self.dirpath / f'epoch{trainer.current_epoch}'
        save_path.mkdir(parents=True, exist_ok=True)

        pl_module.model.save_pretrained(save_path)
        pl_module.model.decoder.tokenizer.save_pretrained(save_path)
...
def train(config):
    ...
    trainer = pl.Trainer(
        ...
        BinModelCheckpoint(dirpath='/mnt/petrelfs/donut/result/donut_base/binmodel_multitask4_otherocrpre')],
    )
